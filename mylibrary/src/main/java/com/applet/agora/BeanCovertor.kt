...
fun mapToVideoEncoderConfiguration(map: Map<*, *>): VideoEncoderConfiguration {
  return VideoEncoderConfiguration().apply {
    ...
    (map["mirrorMode"] as? Number)?.let {
      mirrorMode = intToMirrorModeType(it.toInt()) }
  }
}
...
//          CameraCapturerConfiguration.CaptureFormat(10,10,10)
//  )
//}
...
fun mapToRtcEngineConfig(map: Map<*, *>): RtcEngineConfig {
  ...
  val extensionArr = map["extension"] as JSONArray
  if (extensionArr.size > 0) {
    for (i in extensionArr.indices) {
      rtcConfig.addExtension(extensionArr[i] as String?)
    }
  }
